...
class LinkedList(object):
    ...
    def pivot(self, pivot):
        ...
        #If self.head/curr is not less than pivot, we need to find the first node
        #that is less than pivot to be the new head.
        while curr.data >= pivot:
            self.head = curr.next
            curr.next = None
            largerll.add_node(curr)
            curr = self.head

        #Traverse through the remainder of the ll and if the node is larger or equal
        #Remove the node and add to the largerll
        while curr.next:
            if curr.next.data >=pivot:
                larger_node = curr.next
                curr.next = curr.next.next
                larger_node.next = None
                largerll.add_node(larger_node)
            else:
                curr = curr.next

        curr.next = largerll.head
        self.tail = larger_node
