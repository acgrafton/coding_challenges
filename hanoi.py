...
def move(n, source, helper, target):
    ...
    if n=1:
        pop from source and push to target
    if n=2:
        pop from source, move to helper
        pop from source move to target
        pop from helper move to source
    
    if n=3
        pop from source and move to target #1
        pop from source, move to helper #2
        pop from target, move to helper 2>1 /// recursive call 
        pop from source move to target 3
        pop from helper, move to source 1
        pop from helper move to target 3>2
        pop from from source move to target 3>2>1

    """

    if n == 0 or source.is_empty():
        return
